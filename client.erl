...
start(Name, Master_Registered_Name, Master_Node) ->
	...
	register(Name,Pid),
	subscribe_to_the_master(Master_Registered_Name, Master_Node),
	Pid.
...
send_queryW_message([], Function) -> 'queryW made';
...
send_queryR_message([], Function) -> 'queryR made';
send_queryR_message([H|T],Function) -> 
	H ! {queryR,Function},
	send_queryR_message(T,Function).

%works
send_add_row_message(Tablet_list,Master_Name,Master_Node,Key,Value) ->
...
loop(Tablet_list, Master_Name,Master_Node) -> 
	receive
		...
		{add_row, Key, Value} -> send_add_row_message(Tablet_list,Master_Name,Master_Node,Key,Value),
								 loop(Tablet_list,Master_Name,Master_Node);
