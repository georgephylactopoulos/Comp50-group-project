...
find_problems_with_active_key(AllTablets, TargetCopies, Key) ->
	...
	NotEnoughCopiesProblem = 
		case length(InactiveCopies) - 1 == TargetCopies of
			true -> [];
			false -> 
				io:format("not enough copies of key ~p~n", [Key]),
	...
	InconsistencyProblem = 
		case sets:size(sets:from_list(ActiveCopies ++ InactiveCopies)) of
			1 -> [];
			_ ->
				io:format("inconsistent values of key ~p~n", [Key]),
...
solve_problem(Tablets, Copies, {inconsistent_values, Key, Value, TabletsWithRow}) ->
	lists:map(fun(T) -> gen_server:cast(T, {update_value, Key, Value}) end, TabletsWithRow);
solve_problem(Tablets, Copies, {too_many_active_copies, Key, TabletsWithActive}) ->
	[First | _] = TabletsWithActive,
	gen_server:cast(First, {delete_row, Key});
...
solve_problem(Tablets, Copies, {not_enough_copies, Key, Value, TabletsWithRow}) ->
	...
	TabletsToAdd = lists:sublist(TabletsWithoutRow, length(TabletsWithRow)),
	lists:foreach(fun (T) -> gen_server:call(T, {add_inactive_row, Key, Value}) end, TabletsToAdd).
